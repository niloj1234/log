...
//constants
const VERSION = '0.1.1';
const TOKEN = 'NTMyNDc4OTk2ODk4Nzc1MDQw.DxdFPA.9CNnJBR2ZABhnN3aeBHyfLdW-DY';
const CHANNEL = 'log';
...
bot.on('message', function(message) {
    if(message.author.id != bot.user.id) {
        if (message.channel.type == 'text')
            ...
            message.channel.sendMessage('Sorry im not work in dm!');
    }
});
...
bot.on('messageDelete', function(message) {
    ...
    if(message.channel.type == 'text') {
        ...
        if (log != null)
            log.sendMessage('**[Message Deleted]** ' + message.author.username + ': ' + message.cleanContent);
...
bot.on('messageUpdate', function(oldMessage, newMessage) {
    ...
    if (newMessage.channel.type == 'text' && newMessage.cleanContent != oldMessage.cleanContent) {
        ...
        if (log != null)
            log.sendMessage('**[Message Updated]** *' + newMessage.author.username + '*:\n*Old Message*: ' + oldMessage.cleanContent +
...
bot.on('guildBanAdd', function(guild, user) {
    ...
    if (log != null)
        log.sendMessage('**[Banned]** ' + user.username);
...
bot.on('guildBanRemove', function(guild, user) {
    ...
    var log = guild.channels.find('name', CHANNEL);
    if (log != null)
        log.sendMessage('**[Unbanned]** ' + user.username);
...
bot.on('guildMemberAdd', function(guild, user) {
    ...
    var log = guild.channels.find('name', CHANNEL);
    if (log != null) {
        log.sendMessage('**[Joined]** ' + user.username);
    }
...
bot.on('guildMemberRemove', function(guild, user) {
    ...
    var log = guild.channels.find('name', CHANNEL);
    if (log != null)
        log.sendMessage('**[Left]** ' + user.username);
...
bot.on('guildMemberUpdate', function(guild, oldMember, newMember) {
    ...
    if (log != null) {
        switch(change) {
            case Changes.unknown:
                log.sendMessage('**[User Update]** ' + newMember.username);
                break;
            case Changes.addedRole:
                log.sendMessage('**[User Role Added]** ' + newMember.username + ': ' + addedRole);
                break;
            case Changes.removedRole:
                log.sendMessage('**[User Role Removed]** ' + newMember.username + ': ' + removedRole);
                break;
            case Changes.username:
            ...
            case Changes.nickname:
                ...
                break;
            case Changes.avatar:
                log.sendMessage('**[User Avatar Changed]** ' + newMember.username);
                break;
        }
    }
